...
@section('content')
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form method="GET" action="{{ route('kids.index') }}" class="row g-3" id="search-form">
                <input type="hidden" name="tab" id="search-tab" value="{{ request('tab', 'children') }}">
                <div class="col-md-10">
    ...
    @if(request('search'))
        <div class="alert alert-info border-0 shadow-sm" style="border-radius:12px;">
            ...
            <strong>{{ $children->total() + $adults->total() }}</strong> paciente(s) encontrado(s).
        </div>
    @endif
            ...
            <div class="card border-0 shadow-sm" style="background:#e8f0fe; border-radius:12px;">
                <div class="card-body p-3 d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="fs-4 fw-bold" style="color:#202124;">{{ $children->total() }}</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card border-0 shadow-sm" style="background:#f3e8fe; border-radius:12px;">
                <div class="card-body p-3 d-flex align-items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="fs-4 fw-bold" style="color:#202124;">{{ $adults->total() }}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <ul class="nav nav-tabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link {{ $activeTab === 'children' ? 'active' : '' }}" id="tab-children" data-bs-toggle="tab" data-bs-target="#pane-children" type="button" role="tab">
                <i class="bi bi-person-hearts text-primary"></i> Crianças
                <span class="badge bg-primary ms-1">{{ $children->total() }}</span>
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link {{ $activeTab === 'adults' ? 'active' : '' }}" id="tab-adults" data-bs-toggle="tab" data-bs-target="#pane-adults" type="button" role="tab">
                <i class="bi bi-person" style="color:#7c3aed;"></i> Adultos
                <span class="badge ms-1" style="background:#7c3aed;">{{ $adults->total() }}</span>
            </button>
        </li>
    </ul>
    ...
    <div class="tab-content">
        {{-- Tab Crianças --}}
        <div class="tab-pane fade {{ $activeTab === 'children' ? 'show active' : '' }}" id="pane-children" role="tabpanel">
            @if($children->isNotEmpty())
                ...
                <div class="d-flex justify-content-center mt-3">
                    {{ $children->appends(array_merge(request()->query(), ['tab' => 'children']))->links() }}
                </div>
            @else
        ...
        <div class="tab-pane fade {{ $activeTab === 'adults' ? 'show active' : '' }}" id="pane-adults" role="tabpanel">
            @if($adults->isNotEmpty())
                ...
                <div class="d-flex justify-content-center mt-3">
                    {{ $adults->appends(array_merge(request()->query(), ['tab' => 'adults']))->links() }}
                </div>
            @else
                <div class="alert alert-light mt-3 mb-0">
...
<script>
    document.querySelectorAll('[data-bs-toggle="tab"]').forEach(function(tab) {
        tab.addEventListener('shown.bs.tab', function(e) {
            var tabName = e.target.id === 'tab-adults' ? 'adults' : 'children';
            document.getElementById('search-tab').value = tabName;
        });
    });
</script>
@endpush
