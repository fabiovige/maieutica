...
export default function useChecklistRegisters() {
  ...
  const getChecklistRegister = async (checklist_id, competence_description_id) => {
    ...
    await axios
      .get(
        '/api/checklistregisters?checklist_id=' +
          checklist_id +
  ...
  const getProgressBar = async (checklist_id = 0, totalLevel = 0) => {
    checkIsLoading.value = true
    await axios
      .get('/api/checklistregisters/progressbar/' + checklist_id + '/' + totalLevel)
      .then(response => {
  ...
  const storeChecklistRegister = async data => {
    ...
    checkIsLoading.value = true
    await axios
      .post('/api/checklistregisters', serialized)
  ...
  const storeChecklistRegisterSingle = async data => {
    checkIsLoading.value = true
    await axios
      .post('/api/checklistregisters/single', data)
      .then(response => {
        getProgressBar(data.checklist_id, data.totalLevel)
      })
      .catch(error => {
