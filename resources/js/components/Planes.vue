<template>
  <div>
      ...
      <div class="col-md-12 d-flex justify-content-between">
        ...
        <div v-if="canViewPlane" class="">
          <div v-if="Object.keys(plane).length > 0">
            <button class="btn btn-success mt-3" @click.prevent="viewPdfPlane">
              <i class="bi bi-file-pdf"></i> Visualizar plano
            </button>
          </div>
        </div>

        <div v-if="canCreatePlane" class="">
          <button class="btn btn-dark mt-3" @click.prevent="createPlane">
    ...
    <div class="row">
      <div class="col-md-12 mt-3">
        <table class="table table-sm table-striped mt-2">
          <tbody>
            <tr
              v-for="competence in plane.competences"
              :key="competence.id"
              class="mousePointer"
            ...
            >
              ...
              <td class="customColumn">
                <h5 v-if="competence.note === 0">
                  <span class="badge bg-light text-dark customColumn">Não desenvolvido</span>
                </h5>
                <h5 v-if="competence.note === 1">
                  <span class="badge bg-warning text-dark customColumn">Em desenvolvimento</span>
                </h5>
                <h5 v-if="competence.note === 2">
                  <span class="badge bg-danger customColumn">Parcialmente desenvolvido</span>
                </h5>
                <h5 v-if="competence.note === 3">
                  <span class="badge bg-primary customColumn">Desenvolvido</span>
                </h5>
              </td>
            </tr>
          </tbody>
    ...
    <ul class="nav nav-tabs nav-fill mt-2" id="navTablevelP" role="tablistP">
      <li
        v-for="(data, level_id) in checklist.levels"
        :key="level_id"
        class="nav-item"
        role="presentation"
      >
        <button
          :class="['nav-link', { active: level_id == 1 }]"
      ...
      >
        ...
        <div class="tab-content mt-2" id="myTabContentP">
          <div
            ...
            :id="`${domain}${level_id}P`"
            role="tabpanelP"
            :aria-labelledby="`${domain}${level_id}-tabP`"
          >
            <table
              class="table table-sm table-striped mt-2"
              :id="`${checklist_id}${level_id}${domain}-table`"
            >
              <tbody>
                <tr
                  class="mousePointer"
                  v-for="competence in data.competences[domain]"
                  :key="competence.id"
                  @click.prevent="storePlanes(competence.id)"
                >
                  <td class="customColumnCode">{{ level_id }}{{ domain }}{{ competence.code }}</td>
                  <td>
                    {{ competence.description }}
                  </td>
                  <td style="width: 300px !important">
                    <div class="d-flex justify-content-between">
                      ...
                      <div>
                        <h5 v-if="competence.note === 0">
                          <span class="badge bg-light text-dark customColumn">Não observado</span>
                        </h5>
                        <h5 v-if="competence.note === 1">
                          <span class="badge bg-warning text-dark customColumn"
                            >Em desenvolvimento</span
                          >
                        </h5>
                        <h5 v-if="competence.note === 2">
                          <span class="badge bg-danger customColumn">Não desenvolvido</span>
                        </h5>
                        <h5 v-if="competence.note === 3">
                          <span class="badge bg-primary customColumn">Desenvolvido</span>
                        </h5>
    ...
    setup(props) {
      ...
      function getStatus(competence) {
        ...
        if (currentStatusValue === 3) {
          ...
          if (
            ageInMonths.value >= competence.percentil_25 &&
            ageInMonths.value < competence.percentil_50
          )
            return 'Adiantada'
          if (
            ageInMonths.value >= competence.percentil_50 &&
            ageInMonths.value < competence.percentil_75
          )
            return 'Dentro do esperado'
          if (
            ageInMonths.value >= competence.percentil_75 &&
            ageInMonths.value < competence.percentil_90
          )
            return 'Dentro do esperado'
